...
export const money=(value:number)=>new Intl.NumberFormat("tr-TR",{style:"currency",currency:"TRY",minimumFractionDigits:2}).format(value);
export function monthRange(value?:string){const valid=value&&/^\d{4}-\d{2}$/.test(value)?value:new Date().toISOString().slice(0,7);const [year,month]=valid.split("-").map(Number);const from=`${valid}-01`;const to=new Date(Date.UTC(year,month,1)).toISOString().slice(0,10);return {value:valid,from,to};}
