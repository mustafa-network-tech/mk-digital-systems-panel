...
export default async function Page({searchParams}:{searchParams:Promise<{month?:string}>}){
 ...
 return <><Header title="Taksitler" description="Aylık ödeme planı yalnızca bilgi amaçlıdır; gideri ve kasa bakiyesini etkilemez."/><div className="space-y-5 p-5 lg:p-8"><div className="flex flex-wrap justify-between gap-4"><FinanceNav/><MonthPicker value={range.value} path="/finance/installments"/></div><div><div className="grid gap-3 sm:grid-cols-3"><SummaryCard label="Aylık Genel Toplam" value={total}/><SummaryCard label="Aylık Ödenen" value={paid} tone="green"/><SummaryCard label="Aylık Kalan" value={remaining} tone={remaining>0?"amber":"green"}/></div><div className="mt-2 text-right text-xs muted">Toplam Borç: <b className="text-sm text-slate-700">{money(grandTotalRemaining)}</b><span className="ml-1">(işaretli taksitlerin ödenmemiş tutarı)</span></div></div><p className="text-xs muted">Aylık Genel Toplam, o ay ödenecek bütün taksitleri kapsar. Toplam Borç yalnızca “Toplam borca dahil et” seçili taksitlerin ödenmemiş tutarını gösterir. Bu bilgiler kasa ve gider tablosuna işlenmez.</p><InstallmentForm/><section><h2 className="mb-3 font-bold">Aktif Taksitler</h2><Installments items={installments.filter(item=>item.status!=="completed")}/></section></div></>
}
